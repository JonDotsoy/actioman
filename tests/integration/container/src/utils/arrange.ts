 ...
 * });
 */
export const arrange = <T>(fn: () => T) => {
  const resultPending = Promise.withResolvers<T>();

  const initialize = async () => {
    const result = await fn();

    resultPending.resolve(result);
  };
  const result = () => resultPending.promise;
  ...
  fnResult.initialize = initialize;
